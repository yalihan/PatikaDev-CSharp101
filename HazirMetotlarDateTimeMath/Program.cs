using System;

namespace HazirMetotlarDateTimeMath
{
    class Program
    {
        static void Main(string[] args)
        {
            // DateTime Getting Partition
            Console.WriteLine(DateTime.Now);
            Console.WriteLine(DateTime.Now.Date);
            Console.WriteLine(DateTime.Now.Year);
            Console.WriteLine(DateTime.Now.Month);
            Console.WriteLine(DateTime.Now.Day);
            Console.WriteLine(DateTime.Now.Hour);
            Console.WriteLine(DateTime.Now.Minute);
            Console.WriteLine(DateTime.Now.Second);
            Console.WriteLine(DateTime.Now.Millisecond);

            Console.WriteLine(DateTime.Now.DayOfYear);
            Console.WriteLine(DateTime.Now.DayOfWeek);

            Console.WriteLine(DateTime.Now.TimeOfDay);

            // DateTime ToString
            Console.WriteLine(DateTime.Now.ToLongDateString());
            Console.WriteLine(DateTime.Now.ToShortDateString());

            Console.WriteLine(DateTime.Now.ToLongTimeString());
            Console.WriteLine(DateTime.Now.ToShortTimeString());

            // DateTime Adding
            Console.WriteLine(DateTime.Now.AddYears(2));
            Console.WriteLine(DateTime.Now.AddMonths(2));
            Console.WriteLine(DateTime.Now.AddDays(2));
            Console.WriteLine(DateTime.Now.AddHours(2));
            Console.WriteLine(DateTime.Now.AddMinutes(2));
            Console.WriteLine(DateTime.Now.AddSeconds(2));
            Console.WriteLine(DateTime.Now.AddMilliseconds(2));

            // DateTime Format
            Console.WriteLine(DateTime.Now.ToString("dd"));     // 09
            Console.WriteLine(DateTime.Now.ToString("ddd"));    // Sat
            Console.WriteLine(DateTime.Now.ToString("dddd"));   // Saturday

            Console.WriteLine(DateTime.Now.ToString("MM"));     // 10
            Console.WriteLine(DateTime.Now.ToString("MMM"));    // Oct
            Console.WriteLine(DateTime.Now.ToString("MMMM"));   // October

            Console.WriteLine(DateTime.Now.ToString("yy"));     // 21
            Console.WriteLine(DateTime.Now.ToString("yyyy"));   // 2021

            // Math Kütüphanesi

            // --> İşlem
            Console.WriteLine(Math.Abs(-25));   // 25
            Console.WriteLine(Math.Sin(10) + "\t" + Math.Cos(10) + "\t" + Math.Tan(10));    // sin(10),cos(10),tan(10)
            
            // --> Yuvarlama
            Console.WriteLine(Math.Ceiling(22.3));  // 23 - Yukarı
            Console.WriteLine(Math.Round(22.3));    // 22 - Aşağı(yakın)
            Console.WriteLine(Math.Round(22.7));    // 23 - Yukarı(yakın)
            Console.WriteLine(Math.Floor(22.7));    // 22 - Aşağı

            // --> Max & Min
            Console.WriteLine(Math.Max(2,6));
            Console.WriteLine(Math.Min(2, 6));

            // HighLevel
            Console.WriteLine(Math.Pow(3,4));   // 3^4
            Console.WriteLine(Math.Sqrt(9));    // 9^(1/2)
            Console.WriteLine(Math.Log(9));     // ln9
            Console.WriteLine(Math.Exp(3));     // e^3
            Console.WriteLine(Math.Log10(10));  // log_{10}^10


        }
    }
}
